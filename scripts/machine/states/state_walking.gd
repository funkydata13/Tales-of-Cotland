class_name C_StateWalking extends C_State
...
func _ready():
	...
	if machine.kinematics.nodes.has(C_KinematicNode.E_Type.Ground):
		if machine.kinematics.nodes[C_KinematicNode.E_Type.Ground].nodes.has(C_KinematicNode.E_Type.Walk):
			move_kinematic = machine.kinematics.nodes[C_KinematicNode.E_Type.Ground].nodes[C_KinematicNode.E_Type.Walk] as C_KinematicMove
...
func check_status(_delta:float):
	...
	if not machine.kinematics.is_grounded:
		...
		elif not GameInputs.is_pressed("slow"):
